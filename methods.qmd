...
Let obs_i be the reference value and sim_i the target value for i = 1..n after pairing by date and dropping NA pairs. Define error e_i = sim_i - obs_i, and means mu_obs, mu_sim with standard deviations sigma_obs, sigma_sim.

Core metrics:
- Pearson correlation:
  r = sum((obs_i - mu_obs)(sim_i - mu_sim)) / sqrt(sum((obs_i - mu_obs)^2) * sum((sim_i - mu_sim)^2))
- Linear regression (sim ~ obs):
  sim_i = a + b * obs_i, where b = cov(obs, sim) / var(obs) and a = mu_sim - b * mu_obs
- MAE:
  MAE = (1/n) * sum |e_i|
- RMSE:
...
- Bias:
  Bias = (1/n) * sum e_i
- Relative bias:
  RelBias(%) = 100 * Bias / mu_obs (only reported if mu_obs != 0)
- Lin's concordance correlation coefficient (CCC):
  CCC = (2 * r * sigma_obs * sigma_sim) / (sigma_obs^2 + sigma_sim^2 + (mu_obs - mu_sim)^2)
...
Analyses were performed in R [@r:alan2024]. Data wrangling and visualization follow the tidyverse ecosystem [@tidyverse], and spatial processing relies on the sf framework [@sf].
...
- Jolliffe, I.T., Stephenson, D.B. (eds.). Forecast Verification: A Practitioner’s Guide in Atmospheric Science.

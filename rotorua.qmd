...
Lake Rotorua, situated in the Bay of Plenty region is a \~79.9km2 (by surface area) volcanic crater lake with a maximum depth of 45 meters.

Although coined the largest lake in its region, its mean depth of just 10meters results in a shallow lake classification. Shallow lakes are particularly susceptible to climate driven shifts across short and long term scales. Shallow lakes heat up faster than deep lakes which results in larger seasonal and diurnal temperature fluctuations [@frassl2018]. Shallow lakes also tend to be more susceptible to atmospheric forcing, where wind, air temperature and precipitation patterms which influence water quality and consequent biology. Lake Rotorua experiences cyanobacteria blooms, partially due to its location in the landscape (hydrothermal + agriculture + population density), which receive high nutrient content along with warmer temperatures. In addition to temperature fluxes influencing cyanobacterial fluxes, wind and rain play a role in the intensity and location of these blooms. Wind drives cyanobacterial location, often leading to cyanobloom intensification/concentration in particular areas.

For Lake Rotorua the sites and locations selected were; Limnotrack Buoy (lat:-380550S Lon:1761653E) on lake, NIWA virtual climate station network (VCSN) grid NO:27879 lat:-38.075 lon:176.275, ERA5 statlite data for Lat:-38.114908 and Lon:176.316662, Observational data "Airport" station 1770 (lat:-38.10592 lon:176.31481), Observational data "Town" station 41077 (lat:-38.14635 lon:176.2578).
...
## Map of Rotorua lakes showing data source locations
...
All work was completed in R code [@r:alan2024] following the scripts order stated on README in git repository (00, 01, 02, 03, 04, 05).
...
For precipitation, VCS and Town are the most reliable overall, while ERA5 tends towards higher wet-day values and more false alarms. Buoy precipitation is weakest for event detection (Figure: [precipitation scatter](metrics_stats_results.qmd#fig-scatter-precip)), (Figure: [event outcome](behaviour_results.qmd#fig-precip-events)), (Figure: [Precipitation score](behaviour_results.qmd#fig-Rain-event-scores)), (Figure: [False alarm](behaviour_results.qmd#fig-false-alarm-mag)), (Figure: [Climatology rain](behaviour_results.qmd#fig-climatology-precip)). ERA5 has the potential to be useful for wider scale projects and when needing longer records which are consistent. But for a project which require high accuracy and precision for local rain events (e.g. threshold modelling, runoff timing, loading pulses), ERA5 should be a secondary choice unless bias corrected.
...
### 7-day rolling precipitation totals
...
reactable(
  ...
  striped = TRUE,
  columns = list(
    threshold   = colDef(format = colFormat(digits = 2)),
    POD         = colDef(format = colFormat(digits = 2)),
    FAR         = colDef(format = colFormat(digits = 2)),
    CSI         = colDef(format = colFormat(digits = 2)),
    bias_score  = colDef(format = colFormat(digits = 2))
  )
)
```
:::

...
```{r}
#| label: fig-timeseries-hydrograph
...
#| fig-cap: "Stacked counts of precipitation event outcomes (threshold ≥ 1 mm/day) for each dataset (ERA5, Buoy, Town, VCS) relative to the reference (Airport)."
plot_event_outcomes(
  skill_precip,
...
*Stacked counts of precipitation event outcomes (threshold ≥ 1 mm/day) for each dataset relative to the reference.*
...
#| fig-cap: "Event‑detection skill for precipitation ≥ 1 mm/day, showing Probability detection (POD), false alarm ration (FAR), critical success index (CSI) and bias."
...
*Event‑detection skill for precipitation ≥ 1 mm/day.*
...
#| fig-cap: "Distribution of false‑alarm precipitation amounts (ref ≤ 1 mm/day, target \> 1 mm/day) on a log1p scale."
plot_false_alarm_magnitude(ref_df, targets_list, threshold_mm = precip_event_threshold)
```

*Distribution of false‑alarm precipitation amounts (ref ≤ 1 mm/day, target \> 1 mm/day) on a log1p scale.*

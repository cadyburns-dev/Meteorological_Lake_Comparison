...
*Monthly climatology of daily temperature (°C) for each dataset, showing monthly means (lines) and interquartile ranges (shaded bands). Airport_1770 is the reference; other datasets are overlaid to highlight seasonal offsets and variability.*
...
```{r}
plot_seasonal_scatter(ref_df, targets_list, "Temp_C", season_defs = season_defs)
```

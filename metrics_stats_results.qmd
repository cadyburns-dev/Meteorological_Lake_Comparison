...
#| fig-cap: "This is the caption for the plot."
...
```{r}
#| label: fig-
#| fig-cap: "Rolling 30-day diagnostics for daily temperature (deg C). Panels show bias, correlation, MAE and RMSE."
#| fig-subcap: !expr paste0(names(targets_list), " vs ", params$reference)
purrr::iwalk(
  targets_list,
  ~ print(plot_rolling_diagnostics(ref_df, .x, .y, "Temp_C", window_days = window_days))
) #These track moving-window bias/CCC to surface gradual timing offsets or seasonal drifts.
```

"Rolling 30‑day diagnostics for daily temperature (°C) comparing Virtual climate station network (VCSN) data source to the Airport_1770 reference. Panels show 30‑day moving bias (VCSN − Airport), Pearson correlation (cor), mean absolute error (MAE), and* Root mean square error *(RMSE)."

...
Precipitation, wind, and temperature are unavoidable physical forms when it comes to Lymnological science. Choosing the right meteorological dataset/source can play a critical role in both physical and biological results. However, it is often difficult to know which dataset/source is best to apply. Therefore, by critically comparing and analysing different data sources for a given location, we can gather more confidence in whict data set, which variable and when the dataset is best to use.

## Project description

This project aims to take meteorological data in close proximity from different sources (stations, grided, reanalysis) and evaluate their validity across variables and time.
...
1.  The objective, data sources and the project outputs so the reader can understand what they will get out of the process.

2.  The methods, outlining the steps taken to prepare, align, and compare the metrological datasets.

3.  Viriable diagnostics whcih shows time series plots to visulise data stability and alignment.

<!-- -->

4.  Metrics and stats, which shows the statistical/quantitative diagnostics outlined in the overview.

5.  Seasonal and event behaviour, which shows seasonality preformance and dataset agreement during events.

6.  Rotorua demo gives the reader a visual flow of the project using Lake Rotorua as the example lake.
